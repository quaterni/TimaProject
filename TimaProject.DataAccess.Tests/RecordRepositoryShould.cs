...
namespace TimaProject.DataAccess.Tests
{
    public class RecordRepositoryShould
    {
        ...
        private static readonly Domain.Models.Record _recordInRepo = new Domain.Models.Record(DateTime.MinValue, DateOnly.FromDateTime(DateTime.Now), Guid.NewGuid());
        ...
        {
            var newRecord = new Domain.Models.Record(DateTime.MinValue, DateOnly.FromDateTime(DateTime.Now), Guid.NewGuid());
        ...
        public void ContainsReturnCorrectValue_OnTestData(Domain.Models.Record record, bool expected)
        {
        ...
        public static IEnumerable<object[]> ContainsTestData()
        {
            ...
            yield return new object[] { new Domain.Models.Record(DateTime.Now, DateOnly.FromDayNumber(234), Guid.NewGuid()), false};
        }
        ...
        {
            var record = new Domain.Models.Record(DateTime.MinValue, DateOnly.FromDayNumber(1), Guid.NewGuid());

            var exception = Assert.Throws<NotFoundException<Domain.Models.Record>>(()=> _sut.UpdateItem(record));
        ...
        public void RemoveItem_OnTestData(Domain.Models.Record record, bool expected)
        {
        ...
        {
            yield return new object[] { _recordInRepo, true };
            yield return new object[] { new Domain.Models.Record(DateTime.MinValue, DateOnly.FromDayNumber(1), Guid.NewGuid()), false };
        }

        [Fact]
        public void RaisedRepositoryChanged_AfterAddingNewItem()
        {
            var record = new Domain.Models.Record(DateTime.Now, DateOnly.FromDayNumber(0), Guid.NewGuid());

            var result = Assert.Raises<RepositoryChangedEventArgs<Domain.Models.Record>>(
        ...
        {
            var recordInRepo = new Domain.Models.Record(DateTime.Now, DateOnly.FromDateTime(DateTime.Now), Guid.NewGuid());
            var newNote = new Note("MyNote", Guid.NewGuid(), recordInRepo.Id, DateTime.Now);
            _sut.AddItem(recordInRepo);

            var eventData = Assert.Raises<RepositoryChangedEventArgs<Domain.Models.Record>>(
                e => _sut.RepositoryChanged += e,
                e => _sut.RepositoryChanged -= e,
                () =>
                {
        ...
        {
            var record = new Domain.Models.Record(DateTime.Now, DateOnly.FromDateTime(DateTime.Now), Guid.NewGuid());
