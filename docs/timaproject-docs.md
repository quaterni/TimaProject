...
Во время работы таймера пользователь может менять заголовок, проект и время и дату (время окончания не доступно)
...
_startTime? - назначется при запуске
...
![Классы связанные с Project](./class_diagrams/images/projectform.svg)

###### ProjeсtRepository
Хранилище для проектов

Тесты
- Contains должен возвращать true если проект хранится
- Contains должен возвращать false для не хранящегося проекта

- AddProject должен добавлять новый проект в репозиторий
- AddProject должен выбрасывать ошибку если добавленный проект существует
- AddProject должен выбрасывать ошибку если добавленный проект с таким названием уже есть
- AddProject должен выбрасывать ошибку если добавленный проект с таким id уже есть

- UpdateProject должен обновлять Name для проекта с таким же Id
...
- RemoveProject должен возвращать false для проекта, которого не было в репозитории
...
Тесты
- Name по умолчанию пустой
- Name должно содержать ошибку, если оно пустое (во второй раз)
- Name должно содержать ошибку если в репозитории хранится проект с таким именем
...
- OnProjectEditChanged Корректные изменения из ProjectEdit должны изменять свойство Name
