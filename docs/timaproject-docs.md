...
Запуск таймера (переход из 1 - 2)
Таймер имеет timerStartTime типа DateTime? до запуска таймера переменная null. После запуска таймера устанавливается время из StartTime или текущее время, если StartTime не валидируется. Создается Observable с таймером на каждую секунду, каждую секунду обновляется Time по формуле DateTime.Now - timerStartTime
...
Окончание таймера (переход из 2-1)
Observable завершает работу когда состояние таймера становится NotRunning, по окончанию таймера устанавливается EndTime и обновляется Record. Потом Record очищается и очищаются все поля таймера
...
Тесты
- AddItem должен добавлять Note в репозиторий
- AddItem если Note существует выбрасывает исключение AddingNotUniqueItem
- Contains должен возвращать Note по Id
- UpdateItem обновляет Note
- UpdateItem выбрасывает исключение если Note не найдена
- RemoveItem удаляет запись и возвращает true иначе возвращает false
- GetItems возвращает корректные данные на заданном предикате

- RepositoryChanged вызывается при AddItem
- RepositoryChanged вызывается при UpdateItem
- RepositoryChanged вызывается при RemoveItem
