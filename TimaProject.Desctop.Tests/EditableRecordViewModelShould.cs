...
namespace TimaProject.Desctop.Tests
{
    public class EditableRecordViewModelShould
    {
        ...
        private Domain.Models.Record _record;
        ...
        public EditableRecordViewModelShould()
        {
            ...
            _record = new Domain.Models.Record(
        ...
        public void UpdateRecord_WhenPropertiesIsCorrect()
        {
            ...
            _mockRecordRepository
                ...
                .Callback<Domain.Models.Record>(x=> Assert.Equal(expectedDate, x.Date));
            _sut.Date = expectedDate.ToString();

            _mockRecordRepository
                .Setup(x => x.UpdateItem(It.IsAny<Domain.Models.Record>()))
                .Callback<Domain.Models.Record>(x => Assert.Equal(expectedEndTime, x.EndTime));
            _sut.EndTime = expectedEndTime.ToString();

            _mockRecordRepository
                .Setup(x => x.UpdateItem(It.IsAny<Domain.Models.Record>()))
                .Callback<Domain.Models.Record>(x => Assert.Equal(expectedStartTime, x.StartTime));
            _sut.StartTime = expectedStartTime.ToString();

            _mockRecordRepository
                .Setup(x => x.UpdateItem(It.IsAny<Domain.Models.Record>()))
                .Callback<Domain.Models.Record>(x => Assert.Equal(expectedTitle, x.Title));
            _sut.Title = expectedTitle;

            _mockRecordRepository
                .Setup(x => x.UpdateItem(It.IsAny<Domain.Models.Record>()))
                .Callback<Domain.Models.Record>(x => Assert.Equal(expectedProject, x.Project));
            _sut.Project = expectedProject;

            _mockRecordRepository
                .Verify(x => x.UpdateItem(It.IsAny<Domain.Models.Record>()), Times.Exactly(5));
        ...
        public void IngoreUpdate_OnInvalidProperties()
        {
            ...
            _mockRecordRepository
                .Verify(x => x.UpdateItem(It.IsAny<Domain.Models.Record>()), Times.Never);
        }
        ...
        {
            _mockRecordRepository
                .Setup(x => x.RemoveItem(It.IsAny<Domain.Models.Record>()))
                .Callback<Domain.Models.Record>(x => Assert.Equal(_sut.Record, x));
            _sut.RemoveRecordCommand.Execute(null);

            _mockRecordRepository
                .Verify(x => x.RemoveItem(It.IsAny<Domain.Models.Record>()), Times.Once);
        }
